...
import {
    View,
    Text, TouchableOpacity, StyleSheet, Alert, ScrollView,
} from 'react-native';
...
const FavoritePlaceScreen = ({navigation}) => {
    ...
    const [region, setRegion] = useState({latitude: 37.42362320301861, longitude: -122.08424126696157, latitudeDelta: 0.0922, longitudeDelta: 0.0421});
            ...
            [
                ...
                { text: 'Yes',
                    onPress: () => {
                        ...
                        axios({
                            method: 'delete',
                            url: USER_SERVICE +'/places/'+ userProfile.id +'/'+place.placeId,
                            headers: {
                                'Authorization': 'Bearer '+ userProfile.token
                            }
                        })
    ...
    return(
        <View style={{flex:1}}>
            <View style={{flex: 1}}>
                <ScrollView >
                    {
                        favoritePlaces.length === 0 ? null :
                            favoritePlaces.map((item, i) => (
                                <ListItem key={i} bottomDivider onPress={()=>{showPlaceDetail(item)}}>
                                    <ListItem.Content>
                                        <ListItem.Title style={{fontWeight:'bold'}}>{item.placeName }</ListItem.Title>
                                        <Rating imageSize={15} readonly fractions={1} startingValue={item.placeRating} />
                                        <Text>{item.placeAddress}</Text>
                                    </ListItem.Content>
                                    <Icon name="minus" type={'antdesign'} onPress={()=>{deleteFavoritePlace(item)}}/>
                                </ListItem>
                            ))
            ...
            <View>
                        ...
                        <View style={{flex:1}}>
                            ...
                            >
                                {place!==null && placeDetail!==null?(
                                    ...
                                    >
                                        <Callout
                                            onPress={()=>{addFavoritePlace();}}
                                        >
                                            <View style={{flex:1,length:80, alignItems:'center', justifyContent:'center', padding:5}}>
                                                <Text style={{fontSize:16, fontWeight:'bold'}}>{placeDetail.name}  {placeDetail.rating}/5</Text>
                                                <Text style={{fontSize:13}}>{placeDetail.formatted_address}</Text>
                                                <Text style={{color:'#165470'}}>Tap to add to your favorite places</Text>
                                            </View>
                                        </Callout>
                                    </Marker>
                                ):null}
                ...
                <Overlay overlayStyle={styles.OverlayStyle} isVisible={placeDetailVisible} onBackdropPress={togglePlaceDetail}>
                    <View style={{flex:1}}>
                        <View style={{flex:0.3, alignItems:'center', justifyContent:'center'}}>
                            <Text style={{fontSize:20, fontWeight:'bold'}}>{viewPlace?viewPlace.placeName:''}</Text>
                            <Rating imageSize={15} readonly fractions={1} startingValue={viewPlace?viewPlace.placeRating:''} />
